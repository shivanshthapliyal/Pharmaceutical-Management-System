...
#  The admin will be able to see the order queue at present and all the details pertaining to the same.
    ...
    def printorderqueue():
        cur=con.cursor()
        stmt="""SELECT * FROM pms_try.orders"""
        cur.execute(stmt)
        res=cur.fetchall()
        if(cur.rowcount == 0 ):
            ...
            print("Order Queue :-\n")
            x = PrettyTable(["Order ID","Customer ID","Medicine ID","Status","Date of Order"])
            for i in res:
                x.add_row(i)
            print(x)
...
# This is an important step in any pharmacy as expired medicines are not sold.
    def checkexpirydates():
        cur=con.cursor()
        stmt="""SELECT exp_date FROM pms_try.medicine"""
        cur.execute(stmt)
        res=cur.fetchall()
...
# The admin can supervise the medicines sold within a certain timeframe.
    def checkmedicinessold():
        t1=input("Enter the starting date.")
        t2=input("Enter the ending date.")
        cur=con.cursor()
        stmt="""SELECT  * FROM    pms_try.orders WHERE   From_date >= %s AND To_date   <= %s""",(t1,t2)

        cur.execute(stmt)
        res=cur.fetchall()

        print("Medicines sold between",t1," and ",t2," :-\n")
        x = PrettyTable(["OrderID","CustomerID","MedicineID","Status","DateOfOrder"])
        for i in res:
            print(i)
            x.add_row(i)
        print(x)
